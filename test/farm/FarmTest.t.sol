...
contract FarmTest is TestSetupFarm {
    ...
    function testPauseEmissionsAndTransferRemainingGltr() public {
        vm.roll(startBlock);

        // Setup: Add a pool and deposit some tokens
        farm.add(1, lpTokens[0], true);
        lpTokens[0].mint(address(user1), 1000 ether);
        vm.prank(address(user1));
        lpTokens[0].approve(address(farm), 1000 ether);
        vm.prank(address(user1));
        farm.deposit(0, 1000 ether);
        ...
        uint256 totalPendingBefore = farm.totalPending();
        uint256 expectedRemaining = contractBalance - totalPendingBefore;
        uint256 recipientBalanceBefore = rewardToken.balanceOf(address(this));

        // Test combined pause and transfer
        farm.pauseEmissionsAndTransferRemainingGltr(address(this));

        // Verify emissions are paused (totalPending should be frozen)
        uint256 totalPendingAfter = farm.totalPending();
        vm.roll(block.number + 1000); // Roll forward
        assertEq(
            farm.totalPending(),
            totalPendingAfter,
            "Emissions should be paused"
        );

        // Verify tokens were transferred
        assertEq(
            rewardToken.balanceOf(address(this)),
            recipientBalanceBefore + expectedRemaining,
            "Remaining tokens should be transferred"
        );
        assertEq(
            rewardToken.balanceOf(address(farm)),
            totalPendingAfter,
            "Farm should only have pending rewards left"
        );
    }

    function testPauseEmissionsAndTransferRemainingGltr_ZeroAddress() public {
        vm.expectRevert("Cannot transfer to zero address");
        farm.pauseEmissionsAndTransferRemainingGltr(address(0));
    }

    function testDepositsDisabledAfterPause() public {
        vm.roll(startBlock);

        // Setup: Add a pool and deposit some tokens
        farm.add(1, lpTokens[0], true);
        lpTokens[0].mint(address(user1), 2000 ether);
        vm.prank(address(user1));
        lpTokens[0].approve(address(farm), 2000 ether);

        // First deposit should work (before pause)
        vm.prank(address(user1));
        farm.deposit(0, 1000 ether);
        assertEq(farm.deposited(0, address(user1)), 1000 ether);

        // Pause emissions
        farm.pauseEmissionsAndTransferRemainingGltr(address(this));

        // Second deposit should fail (after pause)
        vm.prank(address(user1));
        vm.expectRevert("Deposits disabled: emissions paused");
        farm.deposit(0, 1000 ether);
    ...
    function testPoolOperationsDisabledAfterPause() public {
        vm.roll(startBlock);

        // Setup: Add a pool before pause
        farm.add(1, lpTokens[0], true);
        assertEq(farm.poolLength(), 1);
        assertEq(farm.poolInfo(0).allocPoint, 1);

        // Pause emissions
        farm.pauseEmissionsAndTransferRemainingGltr(address(this));

        // Adding new pools should fail after pause
        vm.expectRevert("Pool operations disabled: emissions paused");
        farm.add(2, lpTokens[1], true);

        // Pool count should remain unchanged
        assertEq(farm.poolLength(), 1);

        // Modifying existing pool allocation should fail after pause
        vm.expectRevert("Pool operations disabled: emissions paused");
        farm.set(0, 5, true);

        // Pool allocation should remain unchanged
        assertEq(farm.poolInfo(0).allocPoint, 1);
    }
